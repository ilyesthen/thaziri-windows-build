...
import * as dbRouter from './services/DatabaseRouter'
...
function createWindow(): void {
  ...
  mainWindow.on('closed', async () => {
    ...
    if (currentUserId !== null) {
      try {
        const count = await dbRouter.unlockUserSalles(currentUserId)
...
ipcMain.handle('auth:createUserWithPassword', async (_event, data: any) => {
  try {
    return await dbRouter.createUserWithPassword(data)
  } catch (error) {
...
ipcMain.handle('db:createUser', async (_event, data: { name: string; email: string; password: string; role: string; defaultPercentage?: number | null }) => {
  try {
    ...
    const user = await dbRouter.createUserWithPassword(data)
...
ipcMain.handle('db:getUserById', async (_event, id: number) => {
  try {
    return await dbRouter.getUserById(id)
  } catch (error) {
...
ipcMain.handle('db:updateUser', async (_event, data: { id: number; name?: string; email?: string; newPassword?: string | null; role?: string }) => {
  try {
    return await dbRouter.updateUserWithPassword(data)
  } catch (error) {
...
ipcMain.handle('db:deleteUser', async (_event, id: number) => {
  try {
    return await dbRouter.deleteUser(id)
  } catch (error) {
...
ipcMain.handle('db:createNote', async (_event, data: db.CreateNoteInput) => {
  try {
    return await dbRouter.createNote(data)
  } catch (error) {
...
ipcMain.handle('db:getAllNotes', async () => {
  try {
    return await dbRouter.getAllNotes()
  } catch (error) {
...
ipcMain.handle('db:getNoteById', async (_event, id: number) => {
  try {
    return await dbRouter.getNoteById(id)
  } catch (error) {
...
ipcMain.handle('db:updateNote', async (_event, id: number, data: Partial<db.CreateNoteInput>) => {
  try {
    return await dbRouter.updateNote(id, data)
  } catch (error) {
...
ipcMain.handle('db:deleteNote', async (_event, id: number) => {
  try {
    return await dbRouter.deleteNote(id)
  } catch (error) {
...
ipcMain.handle('db:createTask', async (_event, data: db.CreateTaskInput) => {
  try {
    return await dbRouter.createTask(data)
  } catch (error) {
...
ipcMain.handle('db:getAllTasks', async () => {
  try {
    return await dbRouter.getAllTasks()
  } catch (error) {
...
ipcMain.handle('db:getTaskById', async (_event, id: number) => {
  try {
    return await dbRouter.getTaskById(id)
  } catch (error) {
...
ipcMain.handle('db:updateTask', async (_event, id: number, data: any) => {
  try {
    return await dbRouter.updateTask(id, data)
  } catch (error) {
...
ipcMain.handle('db:deleteTask', async (_event, id: number) => {
  try {
    return await dbRouter.deleteTask(id)
  } catch (error) {
...
ipcMain.handle('db:toggleTaskCompletion', async (_event, id: number) => {
  try {
    return await dbRouter.toggleTaskCompletion(id)
  } catch (error) {
...
ipcMain.handle('db:getTagById', async (_event, id: number) => {
  try {
    return await dbRouter.getTagById(id)
  } catch (error) {
...
ipcMain.handle('db:deleteTag', async (_event, id: number) => {
  try {
    return await dbRouter.deleteTag(id)
  } catch (error) {
...
ipcMain.handle('db:get-patients', async (_, limit?: number, offset?: number) => {
  try {
    const patients = await dbRouter.getAllPatients(limit, offset)
...
ipcMain.handle('db:get-patients-count', async () => {
  try {
    const count = await dbRouter.getPatientsCount()
...
ipcMain.handle('db:get-patient', async (_, id: number) => {
  try {
    const patient = await dbRouter.getPatientById(id)
...
ipcMain.handle('db:get-patient-by-code', async (_, code: number) => {
  try {
    const patient = await dbRouter.getPatientByCode(code)
    return { success: true, patient }
  } catch (error: any) {
...
ipcMain.handle('db:search-patients', async (_, searchTerm: string) => {
  try {
    const patients = await dbRouter.searchPatients(searchTerm)
    return { success: true, patients }
  } catch (error: any) {
...
ipcMain.handle('db:create-patient', async (_, data: db.CreatePatientInput) => {
  try {
    const patient = await dbRouter.createPatient(data)
    return { success: true, patient }
  } catch (error: any) {
...
ipcMain.handle('db:update-patient', async (_, data: db.UpdatePatientInput) => {
  try {
    const patient = await dbRouter.updatePatient(data)
    return { success: true, patient }
  } catch (error: any) {
...
ipcMain.handle('db:delete-patient', async (_, id: number) => {
  try {
    await dbRouter.deletePatient(id)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('templates:get-all', async () => {
  try {
    const templates = await dbRouter.getAllTemplates()
...
ipcMain.handle('templates:create', async (_, content: string) => {
  try {
    const template = await dbRouter.createTemplate({ content })
...
ipcMain.handle('templates:update', async (_, { id, content }: { id: number; content: string }) => {
  try {
    const template = await dbRouter.updateTemplate(id, { content })
    return { success: true, template }
  } catch (error: any) {
...
ipcMain.handle('templates:delete', async (_, id: number) => {
  try {
    await dbRouter.deleteTemplate(id)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('honoraires:get-all', async () => {
  try {
    const honoraires = await dbRouter.getAllActesHonoraires()
...
ipcMain.handle('honoraires:create', async (_, data: db.CreateActeHonoraireInput, userId?: number, userRole?: string) => {
  try {
    const honoraire = await dbRouter.createActeHonoraire(data)
    
    // Audit log
    await dbRouter.createAuditLog({
      userId,
      userRole,
...
ipcMain.handle('honoraires:update', async (_, data: db.UpdateActeHonoraireInput, userId?: number, userRole?: string) => {
  try {
    const honoraire = await dbRouter.updateActeHonoraire(data)
...
ipcMain.handle('honoraires:delete', async (_, id: number, userId?: number, userRole?: string) => {
  try {
    ...
    const honoraire = await dbRouter.getActeHonoraireById(id)
    
    await dbRouter.deleteActeHonoraire(id)
...
ipcMain.handle('db:assistant-user:get-or-create', async (_, data: { fullName: string; role: string; percentage: number }) => {
  try {
    const assistantUser = await dbRouter.getOrCreateAssistant(data.fullName, data.role, data.percentage)
...
ipcMain.handle('db:assistant-user:find-by-name', async (_, fullName: string) => {
  try {
    const assistantUser = await dbRouter.findAssistantByName(fullName)
    return { success: true, assistantUser }
  } catch (error: any) {
...
ipcMain.handle('db:assistant-user:update', async (_, data: { id: number; updates: any }) => {
  try {
    const assistantUser = await dbRouter.updateAssistantUser(data.id, data.updates)
    return { success: true, assistantUser }
  } catch (error: any) {
...
ipcMain.handle('db:assistant-user:get-all', async () => {
  try {
    const assistantUsers = await dbRouter.getAllAssistantUsers()
...
ipcMain.handle('db:assistant-session:create', async (_, data: { userId: number; assistantName: string; percentage: number; assistantUserId?: number }) => {
  try {
    const session = await dbRouter.createAssistantSession(data)
...
ipcMain.handle('db:assistant-session:close', async (_, sessionId: number) => {
  try {
    const session = await dbRouter.closeAssistantSession(sessionId)
    return { success: true, session }
  } catch (error: any) {
...
ipcMain.handle('db:assistant-session:get-active', async (_, userId: number) => {
  try {
    const sessions = await dbRouter.getActiveAssistantSessions(userId)
...
ipcMain.handle('db:assistant-session:get-history', async (_, userId?: number) => {
  try {
    const sessions = await dbRouter.getAssistantSessionHistory(userId)
    return { success: true, sessions }
  } catch (error: any) {
...
ipcMain.handle('db:honoraires:get-by-date', async (_, date: string, medecin?: string) => {
  try {
    const honoraires = await dbRouter.getHonorairesByDate(date, medecin)
    return { success: true, honoraires }
  } catch (error: any) {
...
ipcMain.handle('db:honoraires:get-with-patients', async (_, date: string, medecin?: string) => {
  try {
    const records = await dbRouter.getHonorairesWithPatients(date, medecin)
...
ipcMain.handle('db:honoraires:get-patient-visits', async (_, departmentCode: number) => {
  try {
    const visits = await dbRouter.getPatientVisitHistory(departmentCode)
...
ipcMain.handle('db:honoraires:delete', async (_, id: number) => {
  try {
    await dbRouter.deleteHonoraire(id)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('db:honoraires:get-for-patient', async (_, patientCode: number) => {
  try {
    const result = await dbRouter.getHonorairesForPatient(patientCode)
    return result
  } catch (error: any) {
...
ipcMain.handle('db:visit-examinations:get-all', async (_, departmentCode: number) => {
  try {
    const examinations = await dbRouter.getPatientVisitExaminations(departmentCode)
...
ipcMain.handle('db:visit-examinations:get-by-date', async (_, departmentCode: number, visitDate: string) => {
  try {
    const examination = await dbRouter.getVisitExaminationByDate(departmentCode, visitDate)
...
ipcMain.handle('db:visit-examinations:get-all-by-patient', async (_, patientCode: number) => {
  try {
    const examinations = await dbRouter.getAllVisitExaminationsByPatientCode(patientCode)
    return { success: true, examinations }
  } catch (error: any) {
...
ipcMain.handle('db:visit-examinations:create', async (_, data: any) => {
  try {
    const examination = await dbRouter.createVisitExamination(data)
    return { success: true, examination }
  } catch (error: any) {
...
ipcMain.handle('db:visit-examinations:get-by-id', async (_, id: number) => {
  try {
    const examination = await dbRouter.getVisitExaminationById(id)
    return { success: true, examination }
  } catch (error: any) {
...
ipcMain.handle('db:visit-examinations:update', async (_, id: number, data: any) => {
  try {
    const examination = await dbRouter.updateVisitExamination(id, data)
    return { success: true, examination }
  } catch (error: any) {
...
ipcMain.handle('db:visit-examinations:delete', async (_, id: number) => {
  try {
    await dbRouter.deleteVisitExamination(id)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('db:salles:get-all', async () => {
  try {
    const salles = await dbRouter.getAllSalles()
...
ipcMain.handle('db:salles:get-active', async () => {
  try {
    const salles = await dbRouter.getActiveSalles()
    return { success: true, salles }
  } catch (error: any) {
...
ipcMain.handle('db:salles:get-by-id', async (_, id: number) => {
  try {
    const salle = await dbRouter.getSalleById(id)
...
ipcMain.handle('db:salles:create', async (_, data: { name: string; description?: string }) => {
  try {
    const salle = await dbRouter.createSalle(data)
    return { success: true, salle }
  } catch (error: any) {
...
ipcMain.handle('db:salles:update', async (_, id: number, data: { name?: string; description?: string; isActive?: boolean }) => {
  try {
    const salle = await dbRouter.updateSalle(id, data)
    return { success: true, salle }
  } catch (error: any) {
...
ipcMain.handle('db:users:update-current-salle', async (_, userId: number, salleId: number | null) => {
  try {
    const user = await dbRouter.updateUserCurrentSalle(userId, salleId)
    return { success: true, user }
  } catch (error: any) {
...
ipcMain.handle('db:assistant-sessions:update-salle', async (_, sessionId: number, salleId: number) => {
  try {
    const session = await dbRouter.updateAssistantSessionSalle(sessionId, salleId)
    return { success: true, session }
  } catch (error: any) {
...
ipcMain.handle('db:salles:lock', async (_, salleId: number, userData: {
  ...
  try {
    ...
    const salle = await dbRouter.lockSalle(salleId, userData)
...
ipcMain.handle('db:salles:unlock', async (_, salleId: number) => {
  try {
    const salle = await dbRouter.unlockSalle(salleId)
    return { success: true, salle }
  } catch (error: any) {
...
ipcMain.handle('db:salles:check-lock', async (_, salleId: number) => {
  try {
    const result = await dbRouter.isSalleLocked(salleId)
...
ipcMain.handle('db:salles:get-locked', async () => {
  try {
    const salles = await dbRouter.getLockedSalles()
    return { success: true, salles }
  } catch (error: any) {
...
ipcMain.handle('db:salles:unlock-user-salles', async (_, userId: number) => {
  try {
    ...
    const count = await dbRouter.unlockUserSalles(userId)
...
ipcMain.handle('db:payments:checkValidation', async (_, patientCode: number, visitDate: string) => {
  try {
    const result = await dbRouter.checkPaymentValidation(patientCode, visitDate)
    return result
  } catch (error: any) {
...
ipcMain.handle('db:payments:create', async (_, data: {
  ...
  try {
    const payment = await dbRouter.createPaymentValidation(data)
...
ipcMain.handle('db:payments:get-all', async (_, filters?: {
  ...
  try {
    const payments = await dbRouter.getPaymentValidations(filters)
...
ipcMain.handle('db:payments:get-by-date', async (_, date: string) => {
  try {
    const payments = await dbRouter.getPaymentsByDate(date)
    return { success: true, payments }
  } catch (error: any) {
...
ipcMain.handle('db:check-payment-validation', async (_, patientCode: string, visitDate: string) => {
  try {
    ...
    const payments = await dbRouter.getPaymentValidations({
...
ipcMain.handle('db:payments:get-today-by-user', async () => {
  try {
    const paymentsByUser = await dbRouter.getTodayPaymentsByUser()
...
ipcMain.handle('db:payments:get-logs', async (_, paymentId?: number) => {
  try {
    const logs = await dbRouter.getPaymentLogs(paymentId)
...
ipcMain.handle('db:payments:cancel', async (_, 
  ...
  try {
    const payment = await dbRouter.cancelPaymentValidation(paymentId, cancelledBy, cancelledByUserId, cancelledByRole, reason)
    return { success: true, payment }
  } catch (error: any) {
...
ipcMain.handle('db:payments:delete', async (_, 
  ...
  try {
    await dbRouter.deletePaymentValidation(paymentId, deletedBy, deletedByUserId, deletedByRole, reason)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('db:payments:link-to-visit', async (_, 
  ...
  try {
    await dbRouter.linkPaymentsToVisit(patientCode, visitDate, visitId)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('db:payments:delete-all-for-patient-date', async (_, 
  ...
  try {
    const result = await dbRouter.deleteAllPaymentsForPatientDate(patientCode, visitDate, deletedBy, deletedByUserId, deletedByRole, reason)
...
ipcMain.handle('db:payments:getAllByPatient', async (_, patientCode: number) => {
  try {
    const result = await dbRouter.getAllPaymentsByPatient(patientCode)
    return result
  } catch (error: any) {
...
ipcMain.handle('db:queue:sendToRoom', async (_, data: any) => {
  try {
    const queueItem = await dbRouter.sendPatientToRoom(data)
...
ipcMain.handle('db:queue:sendToNurse', async (_, data: any) => {
  try {
    const queueItem = await dbRouter.sendPatientToNurse(data)
    return { success: true, queueItem }
  } catch (error: any) {
...
ipcMain.handle('db:queue:getQueue', async (_, userId: number, userRole: string) => {
  try {
    const queue = await dbRouter.getPatientQueue(userId, userRole)
...
ipcMain.handle('db:queue:getSentItems', async (_, userId: number) => {
  try {
    const items = await dbRouter.getSentQueueItems(userId)
...
ipcMain.handle('db:queue:markSeen', async (_, queueId: number) => {
  try {
    await dbRouter.markQueueItemSeen(queueId)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('db:queue:markCompleted', async (_, queueId: number) => {
  try {
    await dbRouter.markQueueItemCompleted(queueId)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('db:queue:toggleChecked', async (_, queueId: number, isChecked: boolean) => {
  try {
    await dbRouter.toggleQueueItemChecked(queueId, isChecked)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('db:queue:getDailySentCount', async (_, roomId: number, date: string) => {
  try {
    const count = await dbRouter.getDailySentCountForRoom(roomId, date)
    return { success: true, count }
  } catch (error: any) {
...
ipcMain.handle('db:messages:save', async (_, messageData) => {
  try {
    const message = await dbRouter.saveMessage(messageData)
...
ipcMain.handle('db:messages:getRoomMessages', async (_, roomId: number, limit?: number) => {
  try {
    const messages = await dbRouter.getRoomMessages(roomId, limit || 50)
...
ipcMain.handle('db:messages:getUserMessages', async (_, userId: number, limit?: number) => {
  try {
    const messages = await dbRouter.getUserMessages(userId, limit || 50)
    return { success: true, messages }
  } catch (error: any) {
...
ipcMain.handle('db:messages:markAsRead', async (_, messageId: number) => {
  try {
    await dbRouter.markMessageAsRead(messageId)
    return { success: true }
  } catch (error: any) {
...
ipcMain.handle('db:messages:getUnreadCount', async (_, userId?: number, roomId?: number) => {
  try {
    const count = await dbRouter.getUnreadMessageCount(userId, roomId)
    return { success: true, count }
  } catch (error: any) {
...
ipcMain.handle('setup:database', async (_, config: { mode: 'admin' | 'client', databasePath?: string, shareName?: string }) => {
  try {
    ...
    if (config.mode === 'admin') {
      ...
      // Client mode: Save server URL configuration
      if (!config.databasePath) {
        return { success: false, error: 'Server URL is required for client mode' }
      }
      
      // Save server URL (databasePath contains the HTTP server URL)
      require('fs').writeFileSync(configPath, JSON.stringify({
        serverUrl: config.databasePath // This is actually the server URL
      }, null, 2))
      ...
      require('fs').writeFileSync(setupCompletePath, JSON.stringify({
        mode: 'client',
        completedAt: new Date().toISOString(),
        serverUrl: config.databasePath
      }, null, 2))
      
      console.log('✅ Client setup complete. Server URL:', config.databasePath)
